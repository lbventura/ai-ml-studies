...
saved_folder_path = (
    parent_file / f"checkpoints/dcgan-Windows-lr-0.0001-train-iter-{NUMBER_ITERATIONS}"
)
...
real_norm_results = F.softmax(combined_result[:, 0])
normalized_results = torch.stack((real_norm_results, combined_result[:, 1]), dim=1)
print(normalized_results)

# Move tensors to CPU and convert to NumPy arrays
results = normalized_results.cpu().detach().numpy()
x = results[:, 0]
y = results[:, 1]
...
plt.hist(ones, bins=30, alpha=0.5, range=(min_val, max_val), label="Ones", color="red")
