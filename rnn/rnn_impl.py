...
def lossFun(inputs, targets, hprev):
  ...
  loss = 0
  # forward pass
  for t in range(len(inputs)): # the recursion is implicitly controlled by the size of the input
...
n, p = 0, 0
mWxh, mWhh, mWhy = np.zeros_like(Wxh), np.zeros_like(Whh), np.zeros_like(Why) # convenient when performing the update
...
while True:
  ...
  if n % 100 == 0:
    ...
    smooth_loss_p_diff = (list(smooth_losses.values())[-1] - smooth_loss)/smooth_loss
