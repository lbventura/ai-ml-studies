...
filter_mat = np.array([[0, 1, 0], [0, 1, 0], [0, 1, 0]])

vertical_edges = cv2.filter2D(
    image, -1, filter_mat
)  # activates when it turns from white to black
...
def gen_idx_conv1d(in_size, ker_size):
    ...
    def f(dim1, dim2, axis):
        return np.reshape(np.tile(np.expand_dims(np.arange(dim1), axis), dim2), -1)
...
def conv2d(im, ker):
    ...
    if (
        len(im.shape) == 2
    ):  # if the image is a 2D array, it is reshaped by expanding the last dimension
    ...
    if (
        ...
        ker = np.tile(
            np.expand_dims(ker, -1), [1, 1, im_w]
        )  # the same kernel will be applied to all of the channels
    ...
    idx_list_x = gen_idx_conv1d(
        im_x, ker_x
    )  # computes the indices of a 1D conv (cf. idx_conv1d doc)
    idx_list_y = gen_idx_conv1d(im_y, ker_y)

    idx_reshaped_x = repeat_idx_2d(
        idx_list_x, len(idx_list_y), 0
    )  # repeats the previous indices to be used in 2D (cf. repeat_idx_2d doc)
    idx_reshaped_y = repeat_idx_2d(idx_list_y, len(idx_list_x), 1)

    im_reshaped = np.reshape(
        im[idx_reshaped_x, idx_reshaped_y, :], [out_x, ker_x, out_y, ker_y, im_w]
    )  # reshapes
...
res = conv2d(im=image, ker=filter_mat)
print(res)
