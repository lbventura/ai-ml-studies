...
from pyro.distributions import Delta, Normal, Uniform
from pyro.infer import SVI, EmpiricalMarginal, Trace_ELBO, TracePredictive
from pyro.optim import Adam

# Generate random data
np.random.seed(0)
pyro.enable_validation(True)  # type: ignore
pyro.set_rng_seed(1)  # type: ignore
pyro.enable_validation(True)  # type: ignore
...
def regression_training(
    x_data, y_data, loss_fn, optim, NUM_ITERATIONS, regression_model
):
...
def bayesian_model(x_data, y_data, regression_model):
    # weight and bias priors
    w_prior = Normal(torch.zeros(1, 2), torch.ones(1, 2)).to_event(1)
    b_prior = Normal(torch.tensor([[8.0]]), torch.tensor([[1000.0]])).to_event(1)
    f_prior = Normal(0.0, 1.0)
    ...
    scale = pyro.sample("sigma", Uniform(0.0, 10.0))
...
if __name__ == "__main__":
    ...
    # Inspect learned parameters
    for name, value in pyro.get_param_store().items():  # type: ignore
        print(name, pyro.param(name))  # type: ignore

    posterior = svi.step(x_data, y_data)
